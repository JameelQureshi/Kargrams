...
public class MapPointsPlacement : MonoBehaviour {
    ...
    public void PlacePoints(string[] locationsData)
    {
        ...
        {
            Debug.Log("locationLength"+ locationsData.Length);
            locationsData[i] = locationsData[i].Replace(',','.');
            locationsData[i] = locationsData[i].Replace('+', ',');

            UsersFromLocation.message = UsersFromLocation.message +"  "+ locationsData[i];
           ...
           mapPoint.GetComponentInChildren<MapItem>().ARuser = LocationDataManager.instance.users[i];
